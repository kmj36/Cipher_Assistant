...
// 도저히 짜기 힘들어 https://github.com/kokke/tiny-AES-c 인용
...
static void MixColumns(state_t* state)
{
	...
	for (i = 0; i < 4; ++i)
	{
		...
		Tm = xtime(Tm);  (*state)[i][0] ^= Tm ^ Tmp;
		Tm = (*state)[i][1] ^ (*state)[i][2];
		Tm = xtime(Tm);  (*state)[i][1] ^= Tm ^ Tmp;
		Tm = (*state)[i][2] ^ (*state)[i][3];
		Tm = xtime(Tm);  (*state)[i][2] ^= Tm ^ Tmp;
		Tm = (*state)[i][3] ^ t;
...
CString AES(const CString& pt, CString& aeskey)
{
	...
	AES_init_ctx_iv(&ctx, key, iv);
	AES_CBC_encrypt_buffer(&ctx, (uint8_t*)ptch, ptlen);

	CString AESenarr;
	AESenarr.Format(_T("%S"), (LPCTSTR)ptch);
	aeskey.Format(_T("%S"), (LPCTSTR)key);
	return AESenarr;
}